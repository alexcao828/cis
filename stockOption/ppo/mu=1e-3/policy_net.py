...
class policy_net:
    def __init__(self, name: str, num_time_steps, num_features):
        """
        param name: string
        num_time_steps: number of time steps in option
        num_features: number of features for stock
        """
        ...
        self.stocks = tf.placeholder(tf.float32, [self.num_time_steps, self.num_features])
    ...
    def get_action_prob(self, stocks):
        return tf.get_default_session().run(self.act_probs, feed_dict = {self.stocks: stocks})
