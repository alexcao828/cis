...
class ppo_train:
    def __init__(self, policy, old_policy, clip_value=0.2, c_1=1, c_2=0.01):
        ...
        with tf.name_scope('gae/discount_reward'):
            ...
            prob = tf.slice(prob, [T_squeezed-1, 0], [1,2])
            prob = tf.reshape(prob, [2])
        ...
        with tf.name_scope('cross_entropy'):
            ...
            p = p[0:T_squeezed, :]
            y_tiled = tf.tile(tf.reshape(self.y, [1, 2]), [tf.shape(p)[0], 1])
    ...
    def get_grads(self, tokens, y, T, T_slicer, phase):
        ra = tf.get_default_session().run(self.reward_acc, feed_dict={self.policy.tokens: tokens,
                                                                                              self.old_policy.tokens: tokens,
        ...
        if phase == 'train':
            grad, guess, ce, r, r_acc, t_stop = tf.get_default_session().run((self.gradients, self.guess, self.cross_entropy, self.reward, self.reward_acc, self.time_stop),
                                                                              feed_dict={self.policy.tokens: tokens,
                                                                                         self.old_policy.tokens: tokens,
                                                                                         self.y: y, self.T: T, self.T_slicer: T_slicer,
            ...
            guess, ce, r, r_acc, t_stop = tf.get_default_session().run((self.guess, self.cross_entropy, self.reward, self.reward_acc, self.time_stop),
                                                                              feed_dict={self.policy.tokens: tokens,
                                                                                         self.old_policy.tokens: tokens,
                                                                                         self.y: y, self.T: T, self.T_slicer: T_slicer,
                                                                                         self.should_be_gae: discounted_rewards})
            ...
            actProbsOut, classProbsOut = tf.get_default_session().run((self.actProbsOut, self.classProbsOut),
                                                                      feed_dict={self.policy.tokens: tokens,
                                                                                 self.old_policy.tokens: tokens,
                                                                                 self.y: y, self.T: T,
                                                                                 self.T_slicer: T_slicer,
                                                                                 self.should_be_gae: discounted_rewards})

            return guess, ce, r, r_acc, t_stop, actProbsOut, classProbsOut
