...
policy = policy_net('policy',np.amax(x_train)+1, T_end)
old_policy = policy_net('old_policy',np.amax(x_train)+1,T_end)
...
with tf.Session() as sess:
    ...
    racc_val = []
    rtime_val = []
    allValActProbs = np.zeros((num_times_thru_training_set, num_val_episodes, num_time_steps))
    allValClassProbs = np.zeros((num_times_thru_training_set, num_val_episodes, num_time_steps))
    ...
    for ep in range(1, num_iterations + 1):
        ...
        tokens = x_train[i]
        action_probs = policy.get_action_prob(tokens)
        ...
        pe_grads, guess, ce, r, racc, rtime = ppo.get_grads(tokens,y, c * np.ones(1), np.asarray([c - 1, 1]), 'train')
        ...
        if ep % (M*batch_size) == 0:
            ...
            racc_temp = []
            rtime_temp = []
            actProbsEpoch = np.zeros((num_val_episodes, num_time_steps))
            classProbsEpoch = np.zeros((num_val_episodes, num_time_steps))
            ...
            for v in range(num_val_episodes):
                ...
                guess, ce, r, racc, rtime, actProbsOut, classProbsOut = ppo.get_grads(tokens, y, c * np.ones(1), np.asarray([c - 1, 1]), 'val/test')
                ...
                actProbsEpoch[v, :] = actProbsOut
                classProbsEpoch[v, :] = classProbsOut
            ...
            for t in range(num_test_episodes):
                ...
                guess, ce, r, racc, rtime, _, _ = ppo.get_grads(tokens, y, c * np.ones(1), np.asarray([c - 1, 1]), 'val/test')
