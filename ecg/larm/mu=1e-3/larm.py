...
class larm:
    def __init__(self, policy, num_time_steps, mu):
        ...
        num_time_steps: number of time steps in spectrograms
    ...
    def update(self, spectrograms, y):
        _, act_probs, class_probs = tf.get_default_session().run((self.train_opt, self.act_probs, self.class_probs),
                                                         feed_dict={self.policy.spectrograms: spectrograms, self.y: y})
    ...
    def eval(self, spectrograms, y):
        act_probs, class_probs = tf.get_default_session().run((self.act_probs, self.class_probs), feed_dict={self.policy.spectrograms: spectrograms, self.y: y})
