...
class imitate:
    ...
    def update(self, spectrograms, y):
        rewards, rewards_acc = tf.get_default_session().run((self.rewards, self.rewards_acc), feed_dict={self.policy.spectrograms: spectrograms, self.y: y})
        ...
        _, guesses, ce, i = tf.get_default_session().run((self.train_opt, self.guesses, self.cross_entropy, self.imitation),
                                                         feed_dict={self.policy.spectrograms: spectrograms, self.y: y, self.Ts: Ts,
                                                                    self.ideal_policies: ideal_policies})

        return guesses, ce, i, rs_acc, Ts

    def eval(self, spectrograms, y):
        rewards, reward_acc, act_probs = tf.get_default_session().run((self.rewards, self.rewards_acc, self.act_probs), feed_dict={self.policy.spectrograms: spectrograms, self.y: y})
